...
namespace KcpPlayer.Avalonia.Utils
{
    public class RTSPClient
    {
        ...
        public RTSPClient(ILogger logger)
        {
            ...
            keepaliveTimer = new() { Interval = 20 * 1000 };
        ...
        )
        {
            ...
            _logger.Debug("Connecting to {url} ", url);
            _uri = new(url);
            ...
            clientWantsVideo = (
                mediaRequest is MEDIA_REQUEST.VIDEO_ONLY or MEDIA_REQUEST.VIDEO_AND_AUDIO
            );
            clientWantsAudio = (
                mediaRequest is MEDIA_REQUEST.AUDIO_ONLY or MEDIA_REQUEST.VIDEO_AND_AUDIO
            );
            ...
            catch
            {
                ...
                _logger.Warning("Error - did not connect");
                return;
            }
            ...
            rtspClient = new RtspListener(rtspSocket) { AutoReconnect = false };
            ...
            RtspRequest options_message = new RtspRequestOptions { RtspUri = _uri };
        ...
        public void Pause()
        {
            ...
            // Send PAUSE
            RtspRequest pause_message = new RtspRequestPause { RtspUri = _uri, Session = session };
        ...
        public void Play()
        {
            ...
            // Send PLAY
            var playMessage = new RtspRequestPlay { RtspUri = _uri, Session = session };
        ...
        public void Stop()
        {
            ...
            teardown_message.AddAuthorization(
                _authentication,
                _uri!,
                rtspSocket?.NextCommandIndex() ?? 0
            );
        ...
        public void AudioRtpDataReceived(object? sender, RtspDataEventArgs e)
        {
            ...
            {
                _logger.Warning("No parser for RTP payload {audioPayload}", audio_payload);
                return;
            }
        ...
        public void RtcpControlDataReceived(object? sender, RtspDataEventArgs e)
        {
            ...
            while (!rtcpPacket.IsEmpty)
            {
                ...
                {
                    _logger.Warning("Invalid RTCP packet");
                    break;
                }
                ...
                if (rtcpPacket.PacketType == RtcpPacketUtil.RTCP_PACKET_TYPE_SENDER_REPORT)
                {
                    ...
                    _logger.Debug(
                        "RTCP time (UTC) for RTP timestamp {timestamp} is {time} SSRC {ssrc}",
                        rtp_timestamp,
                        time,
                        rtcpPacket.SenderSsrc
                    );
                    _logger.Debug(
                        "Packet Count {packetCount} Octet Count {octetCount}",
                        rtcpPacket.SenderReport.PacketCount,
                        rtcpPacket.SenderReport.OctetCount
                    );
        ...
        private void RtspMessageReceived(object? sender, RtspChunkEventArgs e)
        {
            ...
            _logger.Debug(
                "Received RTSP response to message {originalReques}",
            ...
            {
                _logger.Debug(
                    "Got Error in RTSP Reply {returnCode} {returnMessage}",
                    message.ReturnCode,
                ...
                )
                {
                    ...
                    _logger.Error("Fail to authenticate stoping here");
                    Stop();
                    return;
                }
                ...
                {
                    resend_message.AddAuthorization(
                        _authentication,
                        _uri!,
                        rtspSocket!.NextCommandIndex()
                    );
            ...
            )
            {
                ...
                serverSupportsGetParameter = supportedCommand.Contains(
                    "GET_PARAMETER",
                ...
                describe_message.AddAuthorization(
                    _authentication,
                    _uri!,
                    rtspSocket!.NextCommandIndex()
                );
            ...
            if (message.OriginalRequest is RtspRequestSetup)
            {
                ...
                if (
                    keepaliveTimer != null
                    && message.Timeout > 0
                    && message.Timeout > keepaliveTimer.Interval / 1000
                )
                {
                ...
                if (transportString is not null)
                {
                    ...
                    if (transport.IsMulticast)
                    {
                        ...
                        )
                        {
                            ...
                            if (isVideoChannel)
                            {
                                videoRtpTransport = new MulticastUDPSocket(
                                    multicastAddress,
                                    videoDataChannel.Value,
                                    multicastAddress,
                                    videoRtcpChannel.Value
                                );
                            }
                            else if (isAudioChannel)
                            {
                                audioRtpTransport = new MulticastUDPSocket(
                                    multicastAddress,
                                    videoDataChannel.Value,
                                    multicastAddress,
                                    videoRtcpChannel.Value
                    ...
                    if (transport.LowerTransport == RtspTransport.LowerTransportType.TCP)
                    {
                        ...
                        {
                            tcpTransport.DataChannel =
                                transport.Interleaved?.First ?? tcpTransport.DataChannel;
                            tcpTransport.ControlChannel =
                                transport.Interleaved?.Second ?? tcpTransport.ControlChannel;
                        }
                    }
                    else if (!transport.IsMulticast)
                    {
                        ...
                        {
                            udpSocket.SetDataDestination(
                                _uri!.Host,
                                transport.ServerPort?.First ?? 0
                            );
                            udpSocket.SetControlDestination(
                                _uri!.Host,
                                transport.ServerPort?.Second ?? 0
                            );
                        }
                    }
            ...
            {
                _logger.Debug("Got reply from Play {command} ", message.Command);
            }
        }
        ...
        private void HandleDescribeResponse(RtspResponse message)
        {
            ...
            {
                foreach (
                    Media media in sdp_data.Medias.Where(m => m.MediaType == Media.MediaTypes.video)
                )
                {
                    // search the attributes for control, rtpmap and fmtp
                    // holds SPS and PPS in base64 (h264 video)
                    AttributFmtp? fmtp =
                        media.Attributs.FirstOrDefault(x => x.Key == "fmtp") as AttributFmtp;
                    AttributRtpMap? rtpmap =
                        media.Attributs.FirstOrDefault(x => x.Key == "rtpmap") as AttributRtpMap;
                    ...
                    if (
                        (rtpmap?.EncodingName?.ToUpper().Equals("H265") ?? false)
                        && !string.IsNullOrEmpty(fmtp?.FormatParameter)
                    )
                    {
                        var param = H265Parameters.Parse(fmtp.FormatParameter);
                        if (
                            param.ContainsKey("sprop-max-don-diff")
                            && int.TryParse(param["sprop-max-don-diff"], out int donl)
                            && donl > 0
                        )
                        {
                            h265HasDonl = true;
                    ...
                    if (
                        ...
                        && (
                            (
                                (
                                    fmtpPayloadNumber > -1
                                    && rtpmap.PayloadNumber == fmtpPayloadNumber
                                )
                                || fmtpPayloadNumber == -1
                            )
                    ...
                    if (videoPayloadProcessor is H264Payload && fmtp?.FormatParameter is not null)
                    {
                        ...
                        && fmtp?.FormatParameter is not null
                    )
                    {
                    ...
                    if (videoPayloadProcessor is not null)
                    {
                        ...
                        if (transport != null)
                        {
                            RtspRequestSetup setup_message = new() { RtspUri = video_uri };
                            setup_message.AddTransport(transport);
                            setup_message.AddAuthorization(
                                _authentication,
                                _uri!,
                                rtspSocket!.NextCommandIndex()
                            );
            ...
            {
                foreach (
                    Media media in sdp_data.Medias.Where(m => m.MediaType == Media.MediaTypes.audio)
                )
                {
                    // search the attributes for control, rtpmap and fmtp
                    AttributFmtp? fmtp =
                    ...
                    else
                    {
                        ...
                        audioPayloadProcessor = audio_codec switch
                        {
                            ...
                            "MPEG4-GENERIC" when fmtp?["mode"].ToLower() == "aac-hbr" =>
                                new AACPayload(fmtp["config"]),
                    ...
                    if (audioPayloadProcessor is not null)
                    {
                        ...
                        if (transport != null)
                        {
                            RtspRequestSetup setup_message = new() { RtspUri = audio_uri };
                            setup_message.AddTransport(transport);
                            setup_message.AddAuthorization(
                                _authentication,
                                _uri!,
        ...
        private Uri? GetControlUri(Media media)
        {
            ...
            if (attrib is not null)
            {
                ...
                if (
                    sdp_control.StartsWith("rtsp://", StringComparison.OrdinalIgnoreCase)
                ...
                else
                {
                    ...
                    var baseUriWithTrailingSlash = _uri!.ToString().EndsWith('/')
                        ? _uri
                        : new Uri($"{_uri}/");
                    // relative path
        ...
        private RtspTransport? CalculateTransport(IRtpTransport? transport)
        {
            return rtpTransport switch
            {
                ...
                RTP_TRANSPORT.TCP => new RtspTransport()
                {
                    ...
                    Interleaved =
                        (transport as RtpTcpTransport)?.Channels
                        ?? throw new ApplicationException(
                            "TCP transport asked and no tcp channel allocated"
                        ),
                },
                RTP_TRANSPORT.UDP => new RtspTransport()
                {
                    ...
                    IsMulticast = false,
                    ClientPort =
                        (transport as UDPSocket)?.Ports
                        ?? throw new ApplicationException(
                            "UDP transport asked and no udp port allocated"
                        ),
                },
        ...
        void SendKeepAlive(object? sender, System.Timers.ElapsedEventArgs e)
        {
            ...
            RtspRequest keepAliveMessage = serverSupportsGetParameter
                ? new RtspRequestGetParameter { RtspUri = _uri, Session = session }
            ...
            keepAliveMessage.AddAuthorization(
                _authentication,
                _uri!,
                rtspSocket!.NextCommandIndex()
            );
