...
namespace KcpPlayer.Avalonia.OpenGL
{
    public class ShaderProgram
    {
        ...
        public void SetUniform(string name, int value) =>
            GL.ProgramUniform1(Id, GetUniformLocation(name), value);

        public void SetUniform(string name, float value) =>
            GL.ProgramUniform1(Id, GetUniformLocation(name), value);

        public void SetUniform(string name, in Vector2 value) =>
            GL.ProgramUniform2(Id, GetUniformLocation(name), 1, ref Unsafe.AsRef(in value.X));

        public void SetUniform(string name, in Vector3 value) =>
            GL.ProgramUniform3(Id, GetUniformLocation(name), 1, ref Unsafe.AsRef(in value.X));

        public void SetUniform(string name, in Vector4 value) =>
            GL.ProgramUniform4(Id, GetUniformLocation(name), 1, ref Unsafe.AsRef(in value.X));

        public void SetUniform(string name, in Matrix4x4 value, bool transpose = false) =>
            GL.ProgramUniformMatrix4(
                Id,
                GetUniformLocation(name),
                1,
                transpose,
                ref Unsafe.AsRef(in value.M11)
            );

        public void SetUniform(
            string name,
            in OpenTK.Mathematics.Matrix3 value,
            ...
            GL.ProgramUniformMatrix3(
                ...
                1,
                transpose,
                ref Unsafe.AsRef(in value.Row0.X)
            );

        public int GetUniformLocation(string name)
        {
            ref int loc = ref CollectionsMarshal.GetValueRefOrAddDefault(
                _uniformLocs,
                name,
                out bool exists
            );
        ...
        public void Attach(ShaderType type, string source)
        {
            ...
            if (compileStatus != (int)All.True)
            {
                ...
                throw new InvalidOperationException(
                    $"Failed to attach {type} shader to program '{Name ?? Id.ToString()}':\n\n{infoLog}"
                );
            }
        ...
        public void Link()
        {
            ...
            if (linkStatus != (int)All.True)
            {
                ...
                throw new InvalidOperationException(
                    $"Failed to link shader program '{Name ?? Id.ToString()}':\n\n{infoLog}"
                );
            }
        }
