...
public class VertexFormat
{
    ...
    public static unsafe VertexFormat FromStruct<T>(ShaderProgram shader)
        where T : unmanaged
    {
        ...
        for (int i = 0; i < fields.Length; )
        {
            ...
            int j = i + 1;
            while (j < fields.Length && fields[j].Layout.AttribName == layout.AttribName)
                j++;
            ...
            if (location >= 0)
            {
                ...
                else
                {
                    GL.VertexArrayAttribIFormat(
                        ...
                        location,
                        elemCount,
                        (VertexAttribIntegerType)elemType,
                        offset
                    );
                }
            }
        ...
        static Exception Error(FieldInfo field, string msg) =>
            throw new InvalidOperationException(
                $"{msg} on struct field '{field.DeclaringType!.Name}.{field.Name}'"
            );
    }
...
#pragma warning disable format
    static readonly Dictionary<Type, (VertexAttribType ElemType, int Count)> s_KnownLayoutFormats =
        new()
        {
